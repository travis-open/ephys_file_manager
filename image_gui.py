...
from config import obj_list, ill_list
import umanager
from umanager import snap_save_image, spectra_shut_off
...
class ImageGUI(object):
	def __init__(self, root, parent):
		...
		ttk.Button(mainframe, text="capture, save meta", command=self.snap_and_save_to_ad).grid(column=0, row=2, columnspan=2)
		ttk.Button(mainframe, text="Spectra off", command=spectra_shut_off).grid(column=2, row=2)
		root.bind_all('<F5>', self.change_objective)
		
		root.bind_all('<F6>', self.change_illumination)
	...
	def update_connection(self, core, studio):
